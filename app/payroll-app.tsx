            ...
            <div className="grid-main">
              <div>
                <section className="summary">
                  ...
                  <div className="forecast-values"><div><small>ĐÃ TÍCH LŨY</small><strong>{money(totals.earnedExpected)}</strong></div><div><small>DỰ KIẾN CUỐI KỲ</small><strong>{money(totals.forecastExpected)}</strong></div></div>
                  <div className="summary-bottom"><span>{totals.earnedShifts.length} ca đã hoàn thành</span><span>{totals.futureShifts.length} ca chưa kết thúc</span></div>
                  <div className="summary-stats" aria-label="Tóm tắt kỳ công"><div><Clock3 size={16}/><small>GIỜ ĐÃ LÀM</small><strong>{hours(totals.earnedMinutes)} giờ</strong></div><div><Clock3 size={16}/><small>TỔNG GIỜ KỲ</small><strong>{hours(totals.minutes)} giờ</strong></div><div><CalendarDays size={16}/><small>CA TRONG KỲ</small><strong>{totals.shifts.length} ca</strong></div><div><Wallet size={16}/><small>TIỀN CA CHƯA KẾT THÚC</small><strong>{money(totals.forecastWages)}</strong></div></div>
